...
function answerMatches(answer: string, acceptedAnswer: string) {
  const accepted = normalized(acceptedAnswer).replace(/\$/g, "");
  const cleanedAnswer = answer.replace(/\$/g, "");
  if (!accepted) return false;
  if (cleanedAnswer.includes(accepted)) return true;
  const acceptedNumber = accepted.match(/^-?\d+(?:\.\d+)?%?$/)?.[0];
  if (!acceptedNumber) return false;
  return new RegExp(`(^|\\D)${acceptedNumber.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(\\D|$)`).test(cleanedAnswer);
}
...
export function gradeDiagnosticItem(itemId: string, answerRaw: string, explanationRaw: string, confidence: number, representation?: string): Grade {
  ...
  switch (itemId) {
    case "q1":
      correct = answer.includes("1/4") || answer.includes("quarter");
      if (answer.includes("1/8") || has(combined, ["8 is bigger", "denominator is bigger", "bottom is bigger"])) signals.push({ id: "MISC-FRA-001", reason: "Compared denominators as whole numbers", weight: 32 });
      feedback = correct ? "Good: you identified the larger unit fraction." : "This suggests we should recheck unit fraction size.";
      break;
    case "q2":
      correct = answer.includes("7/12") || answer.includes("seven twelfths");
      if (answer.includes("2/7") || has(combined, ["add the bottom", "add denominators", "tops and bottoms"])) signals.push({ id: "MISC-FRA-002", reason: "Added numerators and denominators separately", weight: 38 });
      ...
      break;
    case "q3":
      correct = answer.includes("4/6") || answer.includes("four sixth");
      ...
      break;
    case "q4":
      correct = answer.includes("0.7") || answer.includes("0.70");
      if (answer.includes("0.56") || has(combined, ["56 is bigger", "more digits", "longer"])) signals.push({ id: "MISC-DEC-001", reason: "Compared decimal digits as whole numbers", weight: 35 });
      feedback = correct ? "Good decimal comparison evidence." : "This suggests decimal place value may need repair.";
      break;
    case "q5":
      correct = /(^|\D)20(\D|$)/.test(answer);
      ...
      break;
    case "q6":
      correct = answer.includes("class b") || answer === "b" || answer.includes("40");
      if (answer.includes("class a") || has(combined, ["18 is more", "more students", "bigger number"])) signals.push({ id: "MISC-PCT-004", reason: "Compared raw counts instead of percentages", weight: 32 });
    ...
    case "q7":
      ...
      break;
    case "q8":
      correct = (answer.includes("0.75") || answer.includes(".75")) && answer.includes("75");
