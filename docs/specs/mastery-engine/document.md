# Rule-based mastery, retention, misconception and recommendation engine
...
- Implement score updates for accuracy, explanation, retention, confidence, hint independence.
- Implement misconception probability status transitions.
- Implement recommendation priority rules.
- Never mark Mastered from correctness alone.
- Weight recent diagnostic evidence strongly enough that corrected understanding visibly changes the student profile.
- Reduce or resolve misconception probabilities when the student provides correct counter-evidence with a reasonable explanation/representation.
...
- Do not average all historical evidence equally. Use recent-weighted scoring so old errors matter, but do not permanently dominate the profile.
- Misconception repair recommendations should trigger for current/new misconception signals or high-probability active misconceptions, not for every low-confidence historical suspicion.

## Status / open questions
Status: done for MVP v1, revised after corrective-diagnostic QA.
