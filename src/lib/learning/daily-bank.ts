import type { PracticeTopic } from "./practice-progress";
...
export const topicLessons: Record<
  Topic,
  {
    goal: string;
    bigIdea: string;
...
export function spreadPick<T>(items: readonly T[], seed: number, salt: number) {
...
export function rotateTemplates(
  source: Template[],
  seed: number,
  count: number,
) {
...
export const topicTeachingDefaults: Record<
  Topic,
...
export function extractFirstFraction(text: string) {
...
export function extractFirstDecimal(text: string) {
  const match = text.match(/\d+\.\d+/);
  return match?.[0] ?? null;
}

export function extractPercent(text: string) {
