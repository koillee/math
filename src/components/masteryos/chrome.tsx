...
const nav = [
  ...
  ["/daily-practice", "Daily Practice", CalendarCheck],
...
export function AppShell({
  ...
  active,
  mode = "owner",
}: { children: React.ReactNode; active?: string; mode?: "owner" | "simple" }) {
  return (
    <main className="min-h-screen bg-[#f6f0e5] text-[#17211f]">
      ...
      <div
        className={cn(
          "relative mx-auto w-full px-4 py-5 md:px-8",
          mode === "owner" ? "flex max-w-7xl gap-6" : "max-w-5xl",
        )}
      >
        {mode === "owner" ? (
            ...
            <nav className="space-y-2">
              {nav.map(([href, label, Icon]) => (
                <Link
                  ...
                  href={href}
                  className={cn(
                    "flex items-center gap-3 rounded-2xl px-4 py-3 text-sm text-[#e5d8c2] transition hover:bg-white/10",
...
export function PageHeader({
  ...
  return (
    <header className="mb-6 rounded-[2rem] border border-[#dfd3c0] bg-white/70 p-6 shadow-sm backdrop-blur">
      <p className="mb-2 text-xs font-semibold uppercase tracking-[0.26em] text-[#94652e]">
        {eyebrow}
      </p>
      <h2 className="font-serif text-4xl font-semibold tracking-tight text-[#10211f] md:text-5xl">
        {title}
      </h2>
      {children && (
        <div className="mt-4 max-w-3xl text-[#53615c]">{children}</div>
      )}
    </header>
  );
}
...
export function Card({
  ...
  return (
    <div
      className={cn(
        "rounded-[1.5rem] border border-[#dfd3c0] bg-white/75 p-5 shadow-sm backdrop-blur",
        className,
      )}
    >
      {children}
...
export function Meter({ value, label }: { value: number; label?: string }) {
  return (
    <div>
      <div className="mb-1 flex justify-between text-xs text-[#64716c]">
        <span>{label}</span>
        <span>{Math.round(value)}%</span>
      </div>
      <div className="h-2 overflow-hidden rounded-full bg-[#e5dccd]">
        <div
          className="h-full rounded-full bg-[#2f6173]"
